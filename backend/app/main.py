...
async def realtime_token(
    ...
    pins Hop to the language the user picked in the app's language menu (defaults to English).
    An optional `{"voice": "cedar"}` body sets Hop's voice (defaults to the server default).
    """
    language = (body or {}).get("language")
    voice = (body or {}).get("voice")
    try:
        data = await mint_client_secret(settings, language, voice, user.id)
