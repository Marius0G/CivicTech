...
//   2) Change HOP_RIVE_SOURCE below to:  require('../assets/pip.riv')
...
export const HOP_RIVE_SOURCE: RiveFileInput | null = null;

/** Artboard + state machine to play (must exist in pip.riv). */
export const HOP_ARTBOARD = 'Hop';
export const HOP_STATE_MACHINE = 'Hop';
 ...
 *   speaking  — Boolean         (true while Hop talks; idle vs talk blend)
 *   celebrate — Trigger         (fire once on success → jump/✨)
 */
export const HOP_PROP = {
  level: 'level',
  speaking: 'speaking',
  celebrate: 'celebrate',
} as const;
